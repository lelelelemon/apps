class StoryRepository
  ...
  def recent
    ...
    stories = base_scope
    hidden_ids = HiddenStory.where(:user_id => @user.id).pluck(:id).uniq
    if @params[:exclude_tags].try(:any?)
      tags = @params[:exclude_tags]
      hidden_tids =  Tagging.where(:tag_id => tags).pluck(:tag_id).uniq
    end
    stories = stories.where.not(:id => hidden_ids).where.not(:id => hidden_tids)
    10.times do |x|
