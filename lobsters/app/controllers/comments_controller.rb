class CommentsController < ApplicationController
  ...
  def index
    ...
    if @user
      ids = HiddenStory.where(:user_id => @user.id).pluck(:story_id)
      @comments = @comments.where("story_id NOT IN (?)", ids)
  ...
  def threads
    ...
    thread_ids = @showing_user.recent_threads(10)
    ...
    t2 = Time.zone.now
    Rails.logger.info "duration1: #{t2 - t1}"
    t1 = t2
    ...
    t2 = Time.zone.now
    Rails.logger.info "duration2: #{t2 - t1}"
    t1 = t2
    ...
    if @user && (@showing_user.id == @user.id)
      ...
      t2 = Time.zone.now
      Rails.logger.info "duration3: #{t2 - t1}"
    end
