class RecurringTodosController < ApplicationController
  ...
  def index
    ...
    find_and_inactivate
    @recurring_todos = current_user.recurring_todos.active.includes(:tags, :taggings).paginate(:page => params[:page])
    @completed_recurring_todos = current_user.recurring_todos.limit(10).completed.includes(:tags, :taggings)
